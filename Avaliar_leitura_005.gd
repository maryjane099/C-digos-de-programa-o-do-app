...
#func _process(delta):
#	pass$"../Resultado_avaliacao_leitura".text = "Burro, leitura errada!"
...
func _on_button_up():
	...
	if leitura_inserida_str[-1] != '0' and leitura_inserida_str[-1] != '5':
		$"../Resultado_avaliacao_leitura".text = "A leitura deve ser múltiplo de 0.05!"
		$"../Resultado_avaliacao_leitura".visible = true
		return
	...
	if abs(leitura_exata - leitura_inserida) < 0.10:
